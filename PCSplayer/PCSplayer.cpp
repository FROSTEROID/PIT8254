...
inline void PlayNote(int octave, int note, int period){
	if (note<0){
...
void PlayChord(int* octaves, int* notes, int period, int notesnumber)
{
	int i = 0;
	OpenSpeaker();
	while (i < Periods[period]* 7 / 8 - Mixdelay/notesnumber)
	{
		for (int j = 0; j < notesnumber; j++)
		{
			SetFreq(NotesInOctaves[octaves[j]][notes[j]]);
			if (i + Mixdelay/notesnumber > Periods[period]* 7 / 8)
				break;
			delay(Mixdelay/notesnumber);
			i+=Mixdelay/notesnumber;
		}
	}
	CloseSpeaker();
	delay(Periods[period] - i);
}
